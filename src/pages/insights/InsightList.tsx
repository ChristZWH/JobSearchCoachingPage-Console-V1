...
import { Table, Button, Space, Input, message, Popconfirm, Typography } from 'antd';
...
export default function InsightList() {
  ...
  const load = useCallback(async (p: number = page, s: string = search) => {
    setLoading(true);
    try {
      const res = await getInsights({ page: p, page_size: 20, search: s || undefined });
  ...
  const handleDelete = async (id: number) => {
    try { await deleteInsight(id); message.success('Deleted'); load(); }
  ...
  if (isOperatorOrAdmin) {
    columns.push({
      ...
      render: (_: unknown, record: IndustryInsight) => (
        <Space>
          <Button type="link" icon={<EditOutlined />} onClick={() => navigate(`/insights/${record.id}/edit`)}>Edit</Button>
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        {isOperatorOrAdmin && <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/insights/new')}>Add</Button>}
      </div>
      <Input.Search
